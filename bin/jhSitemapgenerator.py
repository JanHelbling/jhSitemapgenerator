		...
		for url_to_parse in tmp_urls:
			if not bad_urlschemes_regex.match(url_to_parse):
				...
				if url_to_parse.startswith('http'):
					...
					if p.path != '' and not p.path.startswith('/') and not p.path.startswith('.'):
	...
	def __write_urls(self,url_list):
		...
		if url_list.__len__() != 0:
			try:
				...
				self.fd.write('      xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"\n')
				self.fd.write('      xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"\n')
				self.fd.write('      xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9\n')
				self.fd.write('            http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">\n')
			...
			if self.gz:
				try:
					...
					self.fd.write('</urlset>\n'.encode('utf-8','strict'))
					self.fd.close()
					print("Written {} URL's to sitemap.xml.gz".format(len(scanned_urls)))
				except OSError as e:
					print('Failed to open {}: {}.'.format(e.filename,e.strerror))
					exit_success = False
			if self.plaintext:
